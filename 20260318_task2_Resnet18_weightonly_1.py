"""
Task 2 Ablation A: ResNet18 + Class Weight Only

目标:
1. 去掉第二版中的训练增强噪声，单独验证 class weight 是否有效
2. 保留第二版的 freeze + dropout + cosine 调度，避免与 baseline 混成太多变量
3. 不使用 WeightedRandomSampler，保持 batch 采样分布不变
"""
...
class Config:
    ...
    exp_name = "20260318_task2_Resnet18_weightonly_1"
...
def setup_logger(log_file):
    logger = logging.getLogger("task2_weightonly")
...
def compute_class_weights(excel_path):
    ...
    for cls in range(num_classes):
        weight = total / (num_classes * label_counts[cls])
...
def main():
    ...
    logger.info("消融: 保留 class weight，去掉训练增强，不使用 WeightedRandomSampler")
    ...
    # 严格消融：训练/测试都只做 Resize + Normalize
