"""0502 Risk 实验 A —— SupCon 边界对比学习。

目标
  攻克 baseline 的核心瓶颈: benign 仅 53.5% 识别率。
  加 projection head + Supervised Contrastive Loss,在特征空间显式拉开
  benign / no_tumor / malignant 三个簇的边界。

与 baseline 的差异
  1. 模型: SwinV2SegGuidedRiskSupCon —— forward 返回 3-tuple (含 proj_feat)
  2. 损失: RiskOrdinalSupConLoss —— BCE + seg + lambda_supcon * SupCon
  3. 训练 log: 多记录一个 supcon loss 项
  其他完全沿用 baseline (数据/优化器/阈值搜索/评估流程都一样)。
"""
...
# HF 离线模式 —— 必须在所有 transformers 间接 import 之前
...
from risk_utils_A_supcon import (  # noqa: E402
    SwinV2SegGuidedRiskSupCon,
    RiskOrdinalSupConLoss,
)
...
class Config:
    ...
    exp_name = "20260505_task_risk_A_supcon"
    ...
    model_name = (
        ...
        "10D ExtMeta + Gated Trimodal Fusion + 1D Risk Head + ProjHead(SupCon)"
    )
...
def build_dataloaders(cfg: Config, logger):
    ...
    logger.info("[3/4] 构造 val/test111/test112 dataset ...")
    ...
    logger.info("[3/4] eval dataset OK")
    ...
    logger.info("[4/4] 构建 sampler + DataLoader ...")
...
def build_model(cfg: Config, logger):
    logger.info("构建模型 (BERT + SwinV2-Tiny + ProjHead, 离线加载) ...")
    t0 = time.time()
    common = dict(
        ...
        bert_name="bert-base-chinese", proj_dim=cfg.proj_dim,
    )
    try:
        model = SwinV2SegGuidedRiskSupCon(pretrained=True, **common).to(cfg.device)
    except Exception as exc:
        logger.warning(f"pretrained 加载失败, 使用随机初始化: {exc}")
        model = SwinV2SegGuidedRiskSupCon(pretrained=False, **common).to(cfg.device)
...
def train_one_epoch(model, loader, criterion, optimizer, device, scaler,
                    use_amp, grad_clip):
    model.train()
    stats = {"loss": 0.0, "seg": 0.0, "ord": 0.0, "supcon": 0.0,
             "total": 0, "seg_dices": []}
    ...
    for batch in loader:
        ...
        with torch.amp.autocast(
            ...
            seg_logits, risk_logit, proj_feat = model(
                imgs, metadata=metas,
                input_ids=input_ids, attention_mask=attn_mask)
            loss, seg_l, ord_l, supcon_l = criterion(
                seg_logits, risk_logit, proj_feat,
...
def collect_predictions(model, loader, device):
    ...
    for batch in loader:
        ...
        seg_logits, risk_logit, _proj = model(
            imgs, metadata=metas,
            input_ids=input_ids, attention_mask=attn_mask)
...
def main():
    ...
    logger.info("=" * 72)
    logger.info(f"实验: {cfg.exp_name}  (实验 A — SupCon 边界对比)")
    logger.info(f"模型: {cfg.model_name}")
    logger.info(f"主训练集: {cfg.primary_data_root}")
    logger.info(f"稳定性集: {cfg.secondary_data_root}")
    logger.info(f"Lambda Ord={cfg.lambda_ord} | Lambda SupCon={cfg.lambda_supcon} "
                f"(temperature={cfg.supcon_temperature}, proj_dim={cfg.proj_dim})")
    logger.info(f"Backbone LR: {cfg.backbone_lr} | Head LR: {cfg.head_lr}")
    logger.info(f"Epochs: {cfg.num_epochs} | Batch: {cfg.samples_per_class}")
    ...
    criterion = RiskOrdinalSupConLoss(
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        if epoch % cfg.eval_interval == 0 or epoch == cfg.num_epochs:
            ...
            if val_auc > best_val_auc:
                ...
                logger.info(f"*** 保存最优模型 (Val AUC: {best_val_auc:.4f}, "
                            f"Epoch: {best_epoch}) ***")
    ...
    for src in [__file__,
                ...
                os.path.join(SCRIPT_DIR, "risk_utils_A_supcon.py")]:
