"""
Task 2 Recipe-Tuned: ResNet34 二分类 (良性肿瘤 vs 非肿瘤性息肉)
- 保留弱增强 + class weight 主线思想
- 切换到 AdamW + diff lr + warmup cosine
- 将输入尺寸提升到 320x320
- 作为稳定参考线的升级版
"""
...
class Config:
    ...
    exp_name = "20260319_task2_Resnet34_adamw_warmupcosine_weakaug_classweight_img320_5"
    ...
    seed = 42
    use_amp = True
    loss_name = "CrossEntropyLoss(class_weight + label_smoothing=0.05)"

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    class_names = ["benign", "no_tumor"]
    model_name = "ResNet34 (ImageNet预训练)"
    modification = "AdamW + diff lr + warmup cosine + weakaug + class weight + label smoothing + img320"
...
def build_optimizer(model, cfg):
    backbone_params, head_params = split_backbone_and_head(model, model.fc)
