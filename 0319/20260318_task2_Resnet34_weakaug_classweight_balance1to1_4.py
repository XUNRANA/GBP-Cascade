"""
Task 2 Baseline: ResNet34 二分类 (良性肿瘤 vs 非肿瘤性息肉)
- 良性肿瘤 label=0, 非肿瘤 label=1
- 使用 1:1 平衡版 train/test 数据
- 训练集使用弱数据增强
- 使用类别权重 CrossEntropyLoss
- 测试集仅 Resize + Normalize
- 每3个epoch在测试集上评估
- 保存最优权重(基于macro F1)
"""
...
class Config:
    ...
    train_excel = os.path.join(data_root, "task_2_train_balanced_1to1.xlsx")
    test_excel = os.path.join(data_root, "task_2_test_balanced_1to1.xlsx")

    exp_name = "20260318_task2_Resnet34_weakaug_classweight_balance1to1_4"
...
def main():
    ...
    logger.info("模型: ResNet34 (ImageNet预训练)")
    logger.info("修改: weak augmentation + class weight + balance1to1 数据 + 全参数微调")
    logger.info(f"图像尺寸: train resize {cfg.train_resize}, crop {cfg.img_size}, test {cfg.img_size}")
    ...
    train_transform = transforms.Compose([
        transforms.Resize((cfg.train_resize, cfg.train_resize)),
    ...
    class_weights = build_class_weights(train_dataset.df, cfg.device)
    logger.info(
