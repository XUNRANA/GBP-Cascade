"""
Exp #21d: 1:3 原始数据 + Batch Size 32

核心思路:
  - 与 Exp#18 完全相同 (1:3 原始不平衡数据)
  - Batch Size: 8 → 32
  - 学习率线性缩放: backbone 8e-5, head 8e-4
  - class_weight 自动计算 (约 [1.986, 0.668])
  - 其余超参数不变

vs Exp#18:
  - batch_size: 8 → 32
  - backbone_lr: 2e-5 → 8e-5 (4×)
  - head_lr: 2e-4 → 8e-4 (4×)
"""
...
class Config:
    ...
    exp_name = "20260408_task2_SwinV2Tiny_segcls_21d"
    ...
    modification = (
        "基于Exp#18, "
        "batch_size=32, backbone_lr=8e-5, head_lr=8e-4 (LR 4×缩放), "
        "原始1:3数据, class_weight自动计算"
    )
...
def main():
    ...
    # ── class_weight 自动计算 (基于训练集类别分布) ──
    cls_weights = build_class_weights(train_dataset.df, cfg.class_names, cfg.device)
    logger.info(f"分类类别权重: benign={cls_weights[0]:.4f}, no_tumor={cls_weights[1]:.4f} (自动计算)")
