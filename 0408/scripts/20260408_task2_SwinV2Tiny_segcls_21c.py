"""
Exp #21c: 1:3 原始数据 + Batch Size 16

核心思路:
  - 与 Exp#18 完全相同 (1:3 原始不平衡数据)
  - Batch Size: 8 → 16
  - 学习率线性缩放: backbone 4e-5, head 4e-4
  - class_weight 自动计算 (约 [1.986, 0.668])
  - 其余超参数不变

vs Exp#18:
  - batch_size: 8 → 16
  - backbone_lr: 2e-5 → 4e-5 (2×)
  - head_lr: 2e-4 → 4e-4 (2×)
"""
...
class Config:
    ...
    exp_name = "20260408_task2_SwinV2Tiny_segcls_21c"
    ...
    modification = (
        "基于Exp#18, "
        "batch_size=16, backbone_lr=4e-5, head_lr=4e-4 (LR 2×缩放), "
