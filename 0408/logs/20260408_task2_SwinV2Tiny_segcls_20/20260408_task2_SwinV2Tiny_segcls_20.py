"""
Exp #20: 1:1 类别平衡采样 + Exp#18 全模态门控融合

核心思路:
  - 与 Exp#18 完全相同的模型架构 (SwinV2-Tiny + BERT CrossAttn + GatedTrimodalFusion)
  - 唯一区别: 训练时用 WeightedRandomSampler 实现 benign:no_tumor = 1:1 采样
  - 移除分类 loss 的 class_weight (采样已平衡, 不需要额外加权)

vs Exp#18:
  - 训练采样: shuffle → WeightedRandomSampler (1:1 平衡)
...
class Config:
    ...
    exp_name = "20260408_task2_SwinV2Tiny_segcls_20"
    ...
    model_name = (
        ...
        "10D ExtMeta + Gated Trimodal Fusion + 1:1 Balanced Sampling"
    )
    modification = (
        "与Exp#18相同架构, "
        "训练时用WeightedRandomSampler实现benign:no_tumor=1:1采样, "
        "移除cls class_weight(采样已平衡)"
    )
...
def build_dataloaders(cfg):
    ...
    class_counts = np.bincount(labels)  # [309, 920]
    sample_weights = 1.0 / class_counts[labels]  # benign样本权重高, no_tumor权重低
    sampler = WeightedRandomSampler(
        weights=sample_weights,
        num_samples=len(train_dataset),  # 每epoch采样1229张, ~614 benign + ~615 no_tumor
        replacement=True,
    )

    train_loader = DataLoader(
        train_dataset, batch_size=cfg.batch_size,
        sampler=sampler,  # 替代 shuffle=True
...
def main():
    ...
    n_benign = sum(train_dataset.df["label"] == 0)
    n_notumor = sum(train_dataset.df["label"] == 1)
    logger.info(
        f"训练集: {len(train_dataset)} 张 (benign={n_benign}, no_tumor={n_notumor})"
    )
    logger.info(
        f"采样策略: WeightedRandomSampler 1:1 平衡 "
        f"(每epoch ~{len(train_dataset)//2} benign + ~{len(train_dataset)//2} no_tumor)"
    )
    logger.info(
    ...
    # ── 关键区别: class_weight 设为均等 (采样已平衡) ──
    cls_weights = torch.tensor([1.0, 1.0], dtype=torch.float32, device=cfg.device)
    logger.info(f"分类类别权重: benign={cls_weights[0]:.4f}, no_tumor={cls_weights[1]:.4f} (采样已平衡)")
