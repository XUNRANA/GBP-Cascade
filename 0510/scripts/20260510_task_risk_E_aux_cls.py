"""0510 Risk 实验 E —— 三分类辅助 head + 类别感知决策 + 早停 + 温度校准。

继承 0502 实验 B 的胜利组合 (非对称 target + 早停 + 温度校准),
在模型上额外加 cls_head: 256→128→3 (输出 [mal, ben, nt] logits)。
决策时用类别概率作为硬路由:
  if p_nt  > tau_nt:  → low
...
阈值 + 类别先验联合 grid search,5 个 5 底线约束:
  mal_to_low=0, high_recall>=0.95 (硬)
  ben_to_low<=5%, nt_to_high<=5%  (软,无解时 fallback)
"""
...
from risk_utils_E_aux_cls import (  # noqa: E402
    SwinV2SegGuidedRiskAuxCls,
    RiskOrdinalCEL,
    decide_bands_aux_cls_vec,
    search_thresholds_e,
    evaluate_bands_external,
)
...
# 沿用 B 的非对称 ordinal target
ORDINAL_TARGETS_E = torch.tensor([1.0, 0.35, 0.0], dtype=torch.float32)
...
class Config:
    ...
    exp_name = "20260510_task_risk_E_aux_cls"
    ...
    # 沿用 B 的非对称 ordinal target
    ordinal_targets = ORDINAL_TARGETS_E

    # 实验 E 专属: 三分类辅助 head 的损失权重
    lambda_cls = 0.5

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    class_names = CLASS_NAMES
    model_name = (
        "SwinV2-Tiny@256 + 4ch + Trimodal + 1D Risk Head + 3-class Aux Head "
        "(asym target 1.0/0.35/0.0, λ_cls=0.5, early-stop, temp-scaled)"
    )
...
def build_model(cfg: Config, logger):
    logger.info("构建模型 (BERT + SwinV2-Tiny + Trimodal + AuxCls, 离线加载) ...")
    t0 = time.time()
    common = dict(
    ...
    try:
        model = SwinV2SegGuidedRiskAuxCls(pretrained=True, **common).to(cfg.device)
    except Exception as exc:
        logger.warning(f"pretrained 加载失败: {exc}")
        model = SwinV2SegGuidedRiskAuxCls(pretrained=False, **common).to(cfg.device)
...
def train_one_epoch(model, loader, criterion, optimizer, device, scaler,
                    use_amp, grad_clip):
    model.train()
    stats = {"loss": 0.0, "seg": 0.0, "ord": 0.0, "cls": 0.0,
             "total": 0, "seg_dices": []}
    for batch in loader:
        ...
        with torch.amp.autocast(
            ...
            seg_logits, risk_logit, cls_logits = model(
                imgs, metadata=metas,
                input_ids=input_ids, attention_mask=attn_mask)
            loss, seg_l, ord_l, cls_l = criterion(
                seg_logits, risk_logit, cls_logits, masks, labels, has_masks)
...
def collect_predictions_with_logits(model, loader, device):
    """E 版: 同时收集 risk_logits 和 cls_probs (softmax(cls_logits))。"""
    model.eval()
    all_logits, all_scores, all_cls_probs, all_labels = [], [], [], []
    seg_dices = []
    for batch in loader:
        imgs, masks, metas, input_ids, attn_mask, labels, has_masks = batch
        imgs = imgs.to(device, non_blocking=True)
        ...
        all_cls_probs.append(cls_probs.detach().float().cpu().numpy())
    ...
    return {
        ...
        "cls_probs": np.concatenate(all_cls_probs).astype(np.float64),
...
def main():
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        logger.info(
            ...
            f"| Loss: {tm['loss']:.4f} "
            f"(seg={tm['seg_loss']:.4f} ord={tm['ord_loss']:.4f} cls={tm['cls_loss']:.4f}) "
        ...
        if epoch % cfg.eval_interval == 0 or epoch == cfg.num_epochs:
            ...
            # cls head accuracy 监控
            cls_pred = val_pred["cls_probs"].argmax(axis=1)
            cls_acc = (cls_pred == val_pred["labels"]).mean()
            logger.info(
                f"[Val] ROC-AUC(risk): {val_auc:.4f} | cls_acc: {cls_acc:.4f} "
                f"(p_mal_mean={val_pred['cls_probs'][:,0].mean():.3f} "
                f"p_ben_mean={val_pred['cls_probs'][:,1].mean():.3f} "
                f"p_nt_mean={val_pred['cls_probs'][:,2].mean():.3f})"
            )
    ...
    # ── 在校准后的 val score + cls_probs 上做 5D grid search ──
    e_best = search_thresholds_e(
        val_scores_calib, val_pred["cls_probs"], val_pred["labels"])

    if e_best is None:
        logger.error("E 阈值搜索完全失败,退回简单双阈值。")
        # 极端 fallback: 用 baseline 双阈值,tau_* 全设 1.1 (不触发类别先验)
        e_best = dict(t_low=0.20, t_high=0.50,
                      tau_nt=1.1, tau_mal=1.1, tau_ben=1.1,
                      fallback_used="hard_default")
    t_low, t_high = e_best["t_low"], e_best["t_high"]
    tau_nt, tau_mal, tau_ben = e_best["tau_nt"], e_best["tau_mal"], e_best["tau_ben"]
    chosen_method = (f"E_strict" if e_best.get("fallback_used") is None
                     else f"E_{e_best['fallback_used']}")
    logger.info(f"采用 E 阈值搜索结果 ({chosen_method})")
    logger.info(f"  → t_low={t_low:.3f} t_high={t_high:.3f}")
    logger.info(f"  → tau_nt={tau_nt:.2f} tau_mal={tau_mal:.2f} tau_ben={tau_ben:.2f}")
    if "objective" in e_best:
        logger.info(f"  → obj={e_best['objective']:.4f} "
                    f"ben_to_low={e_best.get('ben_to_low_share', 0):.3f} "
                    f"nt_to_high={e_best.get('nt_to_high_share', 0):.3f} "
                    f"high_p={e_best.get('high_precision', 0):.4f} "
                    f"low_p_nt={e_best.get('low_precision_nt', 0):.4f}")
    logger.info("=" * 72)
    ...
    dump_json({
        ...
        "tau_nt": tau_nt, "tau_mal": tau_mal, "tau_ben": tau_ben,
        "chosen_method": chosen_method,
        "e_search_result": e_best,
    }, thresholds_path)
    logger.info(f"阈值已保存: {thresholds_path}")

    # ── 在测试集上用校准后的 score + cls_probs + 全 5 个阈值算 bands 评估 ──
    eval_results = {}
    for phase, loader, tag in [
        ...
        bands = decide_bands_aux_cls_vec(
            scores_calib, pred["cls_probs"],
            t_low, t_high, tau_nt, tau_mal, tau_ben)

        ev = evaluate_bands_external(
            scores_calib, bands, pred["labels"],
            phase=f"{phase} (E val-thr)",
            extra_meta=dict(
                thresholds=dict(t_low=t_low, t_high=t_high,
                                tau_nt=tau_nt, tau_mal=tau_mal, tau_ben=tau_ben),
                seg_dice=pred["seg_dice"],
                temperature_T=T_fit,
                raw_brier=quick_brier(pred["scores"], pred["labels"]),
                calib_brier=quick_brier(scores_calib, pred["labels"]),
            ))
        logger.info(f"[{phase}] safety: {ev['safety']}")

        # 诊断: 直接在该测试集上搜阈值,看上限
        diag = search_thresholds_e(scores_calib, pred["cls_probs"], pred["labels"])
        ev["diagnostic_search_on_self"] = diag
        if diag is not None:
            logger.info(f"[{phase}] 诊断 — self-search: "
