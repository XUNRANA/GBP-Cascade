"""0510 Risk 实验 H —— Stage1 (tumor/nt) → Stage2 (mal/ben) Cascade + 早停 + 温度校准。

修正 0502 实验 C 的失败。改用:
  Stage1: tumor vs no_tumor (易学,差异大,AUC 应可达 0.99+)
  Stage2: mal vs ben         (在真 tumor 上的精细分类,只在 tumor 子集上算 loss)

决策:
  if p_tumor < tau1: → low      (no_tumor 直接走 low,目标 1+2)
  elif p_mal > tau2: → high     (mal 直接走 high)
  else: → medium                (ben 强制 medium,绝不进 low,目标 3)

温度校准: 2 个独立 head 各做一次。mal head 的校准只在 tumor 样本上 fit
         (与训练分布一致)。
"""
...
# H 不用 ordinal target,但保留占位
ORDINAL_TARGETS_H = torch.tensor([1.0, 0.35, 0.0], dtype=torch.float32)
...
class Config:
    ...
    # H 不用 ordinal target (但保留 Config 字段以兼容 B 主脚本结构)
    ordinal_targets = ORDINAL_TARGETS_H

    # 实验 H 专属: stage2 mal head loss 权重
    lambda_mal = 1.0

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    class_names = CLASS_NAMES
    model_name = (
        "SwinV2-Tiny@256 + 4ch + Trimodal + Cascade(tumor → mal) "
        "(2 BCE heads, mask-weighted stage2, dual temp scaling, cascade decision)"
    )
...
def build_model(cfg: Config, logger):
    logger.info("构建模型 (BERT + SwinV2-Tiny + Trimodal + Cascade, 离线加载) ...")
    t0 = time.time()
    common = dict(
    ...
    try:
        model = SwinV2SegGuidedRiskCascade(pretrained=True, **common).to(cfg.device)
    except Exception as exc:
        logger.warning(f"pretrained 加载失败: {exc}")
        model = SwinV2SegGuidedRiskCascade(pretrained=False, **common).to(cfg.device)
...
def train_one_epoch(model, loader, criterion, optimizer, device, scaler,
                    use_amp, grad_clip):
    model.train()
    stats = {"loss": 0.0, "seg": 0.0, "tumor": 0.0, "mal": 0.0,
             "total": 0, "seg_dices": []}
    for batch in loader:
        ...
        with torch.amp.autocast(
            ...
            seg_logits, tumor_logit, mal_logit = model(
                imgs, metadata=metas,
                input_ids=input_ids, attention_mask=attn_mask)
            loss, seg_l, l_tumor, l_mal = criterion(
                seg_logits, tumor_logit, mal_logit,
...
def collect_predictions_with_logits(model, loader, device):
    """H 版: 收集 tumor / mal 两个 head 的 logits + sigmoid scores。"""
    model.eval()
    all_lt, all_lm, all_pt, all_pm, all_labels = [], [], [], [], []
    seg_dices = []
    for batch in loader:
        imgs, masks, metas, input_ids, attn_mask, labels, has_masks = batch
        imgs = imgs.to(device, non_blocking=True)
...
def main():
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        if epoch % cfg.eval_interval == 0 or epoch == cfg.num_epochs:
            ...
            try:
                ...
                tumor_auc = 0.0
            # mal AUC 只在 tumor 子集 (label != 2) 上算 (与训练时只在 tumor 上学一致)
            ...
            try:
                ...
                mal_auc = 0.0
            val_auc = (tumor_auc + mal_auc) / 2
            logger.info(
                f"[Val] tumor_AUC: {tumor_auc:.4f} | mal_AUC(on tumor): {mal_auc:.4f} | "
                f"avg: {val_auc:.4f}"
            )
    ...
    # ── Dual Temperature Scaling: tumor head 和 mal head 各做一次 ──
    val_pred = collect_predictions_with_logits(model, val_loader, cfg.device)
    ...
    # mal head 校准: 只在 tumor 子集上 fit (与训练分布一致)
    ...
    logger.info("=" * 72)
    logger.info(f"[Temperature Scaling] T_tumor = {T_tumor:.4f}, T_mal = {T_mal:.4f}")
    logger.info("=" * 72)

    # ── Cascade 阈值搜索 ──
    h_best = search_thresholds_cascade(
        p_tumor_calib_val, p_mal_calib_val, val_pred["labels"])

    if h_best is None:
        logger.error("H cascade 阈值搜索完全失败,使用 (tau1=0.5, tau2=0.5) 作为兜底。")
        tau1, tau2 = 0.5, 0.5
        chosen_method = "hard_default"
    else:
        tau1, tau2 = h_best["tau1"], h_best["tau2"]
        chosen_method = (f"H_strict" if h_best.get("fallback_used") is None
                         else f"H_{h_best['fallback_used']}")
        logger.info(f"采用 H cascade 阈值 ({chosen_method}): "
                    f"tau1={tau1:.3f} tau2={tau2:.3f}")
        logger.info(f"  → tumor_recall={h_best['tumor_recall']:.4f} "
                    f"high_p={h_best['high_precision']:.4f} "
                    f"low_p_nt={h_best['low_precision_nt']:.4f} "
                    f"ben_to_low={h_best['ben_to_low_share']:.3f} "
                    f"nt_to_high={h_best['nt_to_high_share']:.3f} "
                    f"obj={h_best['objective']:.4f}")
    logger.info("=" * 72)
    ...
    dump_json({
        ...
        "h_search_result": h_best,
    }, thresholds_path)
    logger.info(f"阈值已保存: {thresholds_path}")

    # ── 在测试集上用校准后的 (p_tumor, p_mal) + cascade 阈值 (tau1, tau2) 评估 ──
    eval_results = {}
    for phase, loader, tag in [
        ...
        # 用 p_mal_calib 作为整体 risk score 占位 (做 distribution / calibration)
