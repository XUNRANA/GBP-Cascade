...
继承 0502 实验 B 的全部胜利组合 (非对称 target + 早停 + 温度校准),
新增 pairwise margin ranking loss 强制 sigmoid(score) 满足:
    score(mal) > score(ben) + margin
    score(ben) > score(nt)  + margin
margin 在 post-sigmoid score 空间度量 (margin=0.2),λ_rank=0.3。

阈值搜索新增 2 条软底线:
    benign_to_low <= 5%   (本轮目标 3:良性息肉不应在 low band)
    nt_to_high   <= 5%    (本轮目标 1:正常人不应被推手术)
若软底线无解,自动 fallback 到 0.10 / 完全去掉软底线。
"""
...
from risk_utils_G_margin import (  # noqa: E402
    RiskOrdinalMarginLoss, search_thresholds_g,
)
...
ORDINAL_TARGETS_G = torch.tensor([1.0, 0.35, 0.0], dtype=torch.float32)
...
class Config:
    ...
    exp_name = "20260510_task_risk_G_margin"
    ...
    # 沿用 B 的非对称 ordinal target
    ordinal_targets = ORDINAL_TARGETS_G

    # 实验 G 专属: margin ranking
    lambda_rank = 0.3
    margin = 0.2
    ...
    model_name = (
        ...
        "(asym target 1.0/0.35/0.0, margin ranking λ=0.3 m=0.2, early-stop, temp-scaled)"
    )
...
def train_one_epoch(model, loader, criterion, optimizer, device, scaler,
                    use_amp, grad_clip):
    model.train()
    stats = {"loss": 0.0, "seg": 0.0, "ord": 0.0, "rank": 0.0,
             "total": 0, "seg_dices": []}
    for batch in loader:
        ...
        with torch.amp.autocast(
            ...
            loss, seg_l, ord_l, rank_l = criterion(
...
def main():
    ...
    criterion = RiskOrdinalMarginLoss(
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        logger.info(
            ...
            f"| Loss: {tm['loss']:.4f} "
            f"(seg={tm['seg_loss']:.4f} ord={tm['ord_loss']:.4f} rank={tm['rank_loss']:.4f}) "
    ...
    # ── 在校准后的 val score 上做约束阈值搜索 (G 新软底线优先) ──
    g_best = search_thresholds_g(val_scores_calib, val_pred["labels"])
    base_search = search_risk_thresholds_constrained(
        val_scores_calib, val_pred["labels"])

    if g_best is not None and g_best.get("fallback_used") not in ("no_soft_floor",):
        # G 软底线生效
        t_low, t_high = g_best["t_low"], g_best["t_high"]
        chosen_method = ("G_strict" if g_best.get("fallback_used") is None
                         else f"G_{g_best['fallback_used']}")
        logger.info(f"采用 G 阈值搜索结果 ({chosen_method})")
        logger.info(f"  → t_low={t_low:.3f}, t_high={t_high:.3f}, "
                    ...
                    f"high_p={g_best['high_precision']:.4f}, "
                    f"low_p_nt={g_best['low_precision_nt']:.4f}")
    else:
        # G 无解,退回 baseline 阈值
        t_low, t_high = base_search["t_low"], base_search["t_high"]
        chosen_method = "baseline_fallback"
        logger.warning(f"G 软底线无解,退回 baseline 阈值")
        logger.info(f"  → t_low={t_low:.3f}, t_high={t_high:.3f}")
    logger.info("=" * 72)
    ...
    dump_json({
        ...
        "lambda_rank": cfg.lambda_rank, "margin": cfg.margin,
        "brier_before_calib": brier_before,
        "brier_after_calib": brier_after,
        "t_low": t_low, "t_high": t_high,
        "chosen_method": chosen_method,
        "g_strict_result": g_best,
        "baseline_search_metrics": base_search["metrics"],
        "baseline_search_profile": base_search["profile_used"],
    }, thresholds_path)
    ...
    code_snapshots = [
        __file__,
        os.path.join(SCRIPT_DIR, "risk_utils_G_margin.py"),
