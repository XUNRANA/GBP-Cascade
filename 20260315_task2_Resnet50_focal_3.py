"""
Task 2 V3-ResNet50: ResNet50 + Focal Loss + 平衡采样
改进点（相对V2 ResNet18）：
1. ResNet50 替代 ResNet18（更深的特征提取，2048维特征）
2. Focal Loss（聚焦难分样本）
3. WeightedRandomSampler 平衡采样
4. 冻结 layer1-3，仅微调 layer4 + fc
5. Dropout(0.5) + AdamW + CosineAnnealingLR
"""
...
class Config:
    ...
    exp_name = "20260315_task2_Resnet50_focal_3"
...
def setup_logger(log_file):
    logger = logging.getLogger("task2_v3_r50")
...
class ResNet50WithDropout(nn.Module):
    """ResNet50 冻结 layer1-3，微调 layer4 + fc"""
    def __init__(self, num_classes=2, dropout=0.5):
        ...
        self.fc = nn.Linear(2048, num_classes)  # ResNet50 输出2048维
...
def main():
    ...
    logger.info(f"模型: ResNet50 (冻结layer1-3, 微调layer4+fc)")
    logger.info(f"改进: Focal Loss(gamma={cfg.focal_gamma}) + WeightedRandomSampler + Dropout({cfg.dropout})")
    ...
    # 模型
    model = ResNet50WithDropout(num_classes=2, dropout=cfg.dropout).to(cfg.device)
