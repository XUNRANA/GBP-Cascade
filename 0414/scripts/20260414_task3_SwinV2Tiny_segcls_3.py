"""
0414 Exp-C: Seg-Guided Attention + 10D Clinical Metadata + Ordinal Score

在 Exp-B 基础上新增:
  1. Seg-Guided Attention: 用分割概率图对 f2 做空间注意力池化,
     聚焦病灶区域 (预期改善 benign recall)
  2. 10D 临床 metadata (age/gender/size_mm/size_bin/flow_bin/morph_bin
     + echo_type/lesion_count/wall_thickness/us_diameter) → MLP → 64D
  3. 融合: concat(seg_guided_feat=256D, meta_feat=64D) = 320D
     → cls_head + ord_head

保留 Exp-B 全部机制:
  - CostSensitiveLoss 非对称代价矩阵
  - OrdinalScoreHead 0~1 风险分数
  - malignant 分割 loss 自动置零
  - BalancedBatchSampler 2:2:4
  - 患者级 val 切分
  - 双阈值搜索 + 临床指标
"""
...
# ═════════════════════════════════════════════════════════════
#  3. BalancedBatchSampler (同 Exp-B)
# ═════════════════════════════════════════════════════════════

class BalancedBatchSampler(Sampler):
    """每个 batch 固定包含各类指定数量的样本. 默认 2:2:4."""
...
class GBPDataset0414WithMeta(Dataset):
    """0414 dataset: 4ch + lesion seg target + 3-class label + 10D metadata."""
...
# ═════════════════════════════════════════════════════════════
#  5. 模型: SwinV2 + Seg-Guided Attention + Metadata + Ordinal
# ═════════════════════════════════════════════════════════════

class SwinV2SegGuidedOrdinalMetaModel(nn.Module):
    """
    SwinV2-Tiny@256 + 4ch + UNet seg decoder
    + Seg-Guided Attention (在 f2 上做注意力池化)
    + 10D Metadata MLP
    + 3-class cls head + ordinal score head (0~1)

    与 Exp-B 区别:
      - 分类不再用 f3 的 GAP, 而是用 seg probability 引导的 f2 注意力池化 → 256D
      - 新增 metadata encoder: 10D → 64 → 64
      - 融合: concat(256 + 64) = 320D → cls_head + ord_head
    """
    ...
    def __init__(self, num_seg_classes=2, num_cls_classes=3, meta_dim=10,
        ...
        # ── Seg-Guided Attention: project f2 (384ch) → 256D ──
        self.cls_proj = nn.Sequential(
            nn.Conv2d(fc[2], 256, 1), nn.GroupNorm(8, 256), nn.GELU(),
        )

        # ── Metadata encoder: 10D → 64D ──
        fusion_in = 256
        ...
        # ── Ordinal score head: 320 → 1 → sigmoid ──
    ...
    def forward(self, x, metadata=None):
        ...
        # ── Seg-Guided Attention on f2 ──
        seg_prob = F.softmax(seg_logits, dim=1)[:, 1:2]  # lesion channel (B, 1, H, W)
        attn = F.interpolate(seg_prob, size=f2.shape[2:], mode="bilinear", align_corners=False)
        attn = attn + 0.1  # baseline attention, 避免完全忽略非病灶区域
        attn = attn / (attn.sum(dim=(2, 3), keepdim=True) + 1e-6)

        f2_proj = self.cls_proj(f2)                      # (B, 256, h, w)
        ...
        if self.meta_encoder is not None and metadata is not None:
            ...
            cls_feat = torch.cat([cls_feat, meta_feat], dim=1)  # (B, 320)

        # ── Cls + Ordinal ──
        cls_logits = self.cls_head(cls_feat)              # (B, 3)
        ord_score = torch.sigmoid(self.ord_head(cls_feat).squeeze(-1))  # (B,)
...
def train_one_epoch(model, dataloader, criterion, optimizer, device,
    ...
    for imgs, masks, labels, has_masks, metas in dataloader:
        ...
        with torch.amp.autocast(device_type="cuda", enabled=use_amp):
            seg_logits, cls_logits, ord_score = model(imgs, metadata=metas)
...
class Config:
    ...
    exp_name = "20260414_task3_SwinV2Tiny_segcls_3"
    ...
    num_seg_classes = 2
    num_cls_classes = 3
    meta_dim = 10          # 10D clinical features
    meta_hidden = 64
    meta_dropout = 0.2
    cls_dropout = 0.4
    pretrained = True
    ...
    model_name = (
        ...
        "10D_Meta + 3ClsCostSensitive + OrdinalScore"
    )
    modification = (
        "Exp-C: 在Exp-B基础上新增 Seg-Guided Attention (分割概率引导f2注意力池化) + "
        "10D临床metadata(age/gender/size/echo/flow/morph/...) → MLP → 64D融合, "
        "预期改善 benign recall"
    )
...
def build_dataloaders(cfg):
    ...
    # 用 train_df fit meta stats, val/test 复用
    ...
    meta_stats = train_dataset.meta_stats  # 训练集 fit 的统计量
...
def main():
    ...
    logger.info(f"Meta features: {train_dataset.meta_feature_names}")
    logger.info(f"Meta stats: { {k: {kk: round(vv, 3) for kk, vv in v.items()} for k, v in train_dataset.meta_stats.items()} }")

    # ── 模型 ──
    try:
        model = SwinV2SegGuidedOrdinalMetaModel(
