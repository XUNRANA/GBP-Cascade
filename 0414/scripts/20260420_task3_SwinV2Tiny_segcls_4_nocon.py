...
class Config:
    ...
    exp_name = "20260420_task3_SwinV2Tiny_segcls_4_nocon_r6"
    ...
    lambda_cls = 2.0
    lambda_ord = 0.5
    lambda_con = 0.0      # SupCon loss weight (ablation: off)
    ...
    model_name = (
        ...
        "10D_Meta + FocalCostSensitive + OrdinalScore (No SupCon)"
    )
    modification = (
        "Ablation: 关闭 SupCon 对比损失 (lambda_con=0), 其余保持与 segcls_4 一致 — "
        "Focal代价敏感损失 + 调整代价矩阵 + 采样2:3:3 + ordinal目标微调"
    )
