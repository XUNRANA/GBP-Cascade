"""
Exp #5: DeiT-III-Small + ROI裁剪 + 病灶Mask 4通道
- 数据高效 ViT，专为中小数据集优化
...
class Config:
    ...
    exp_name = "20260323_task2_DeiT3Small_roi4ch_5"
    ...
    model_name = "DeiT-III-Small (ImageNet pretrained, 4ch adapted)"
...
def build_model():
    model = timm.create_model("deit3_small_patch16_224.fb_in1k", pretrained=True, num_classes=2)
...
def build_optimizer(model, cfg):
    head = model.head if isinstance(model.head, torch.nn.Linear) else model.head.fc
